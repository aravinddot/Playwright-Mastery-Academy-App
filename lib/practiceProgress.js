...
export const PRACTICE_TASK_STATE_STORAGE_KEY = "practice_module_task_state_v1";
export const WAIT_NAVIGATION_STATUS_SESSION_KEY = "pma_wait_navigation_status";
...
export function readPracticeTaskState() {
  if (typeof window === "undefined") return {};
  const raw = window.localStorage.getItem(PRACTICE_TASK_STATE_STORAGE_KEY);
  if (!raw) return {};
  return parseStoredProgress(raw);
}
...
export function writePracticeTaskState(taskStateMap) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(PRACTICE_TASK_STATE_STORAGE_KEY, JSON.stringify(taskStateMap));
}
...
export function clearPracticeProgress() {
  ...
  window.localStorage.removeItem(PRACTICE_TASK_STATE_STORAGE_KEY);
  window.sessionStorage.removeItem(WAIT_NAVIGATION_STATUS_SESSION_KEY);
}
